"""
Extract bottom-phase snippets from squat video(s) using a trained PhaseTCN model.
...
Output format is identical to extract_bottom_squat.py so that train_squat_bottom.py
can consume the saved .npz files without modification.

Usage:
  python squat/extract_bottom_squat_phase.py \
      --video path.mp4 \
      --label good_squat \
      --model squat/models/squat_phase_tcn.pt

  python squat/extract_bottom_squat_phase.py \
      --video dataset/squat/good_squat/ \
      --label good_squat \
      --model squat/models/squat_phase_tcn.pt
...
def load_phase_model(model_path: str, device: str) -> Tuple[PhaseTCN, int, int]:
    ...
    :param model_path: path to .pt checkpoint saved by train_squat_phase.py
...
def predict_phases(
    ...
    are assigned the last known prediction.
    ...
    """
    T = len(feature_seq)
    vote_counts = np.zeros((T, 2), dtype=np.int32)  # vote_counts[t, c] = votes for class c at frame t

    with torch.no_grad():
        # stride=1 for dense per-frame coverage
        for start in range(0, max(1, T - window + 1)):
            end = start + window
            chunk = feature_seq[start:end]           # (W, F) — may be shorter at tail
            if len(chunk) < window:
                break

            x = torch.from_numpy(chunk).float().unsqueeze(0).to(device)  # (1, W, F)
            logits = model(x)                         # (1, W, num_classes)
    ...
    # Frames with no votes keep 0 (eccentric) as default
    per_frame = vote_counts.argmax(axis=1).astype(np.int64)

    # Fill any zero-vote frames from their neighbour to avoid isolated gaps
...
def find_bottom_events(
    per_frame: np.ndarray,  # (T,) predicted phase
    min_gap: int = 18,
) -> List[int]:
    """Find frame indices where phase transitions from eccentric → concentric.

    A transition at frame t means per_frame[t-1] == ECCENTRIC and
    per_frame[t] == CONCENTRIC.  min_gap enforces a minimum number of frames
    between successive detections to avoid detecting the same bottom twice.

    :param per_frame: per-frame phase predictions
    :param min_gap: minimum frames between consecutive bottom events
    :returns: list of frame indices (the first concentric frame after each transition)
    """
    events: List[int] = []
...
# ─────────────────────────────────────────────────────────────────────────────
# NPZ / MP4 writers  (identical format to extract_bottom_squat.py)
# ─────────────────────────────────────────────────────────────────────────────
...
def write_snippet_npz(
    ...
    """Save snippet keypoints + metadata as a compressed NPZ.

    Format matches extract_bottom_squat.py so that train_squat_bottom.py
    can load the files without modification.

    :param out_path: destination .npz path
    :param kpts_list: list of (33, 4) arrays or None per frame
    :param label: form label string
    :param fps: video frame rate
    :param event_frame: bottom event frame index
    :param start_frame: first frame of snippet
    :param end_frame: last frame of snippet
    """
    T = len(kpts_list)
...
def write_snippet_mp4(
    ...
    """Write frames to an MP4 file with label overlay.

    :param out_path: destination .mp4 path
    :param frames: list of BGR frames
    :param fps: frame rate
    :param size_wh: (width, height)
    :param label: label text for overlay
    :param snip_id: snippet id for overlay
    """
...
def landmarks_to_array(lm) -> np.ndarray:
    """Convert MediaPipe landmarks to a (33, 4) float32 array (x, y, z, vis).

    :param lm: MediaPipe landmark list (length 33)
    :returns: numpy array of shape (33, 4)
    """
...
def collect_video_paths(path: str) -> List[str]:
    """Return sorted video paths from a file or directory.

    :param path: file path or directory path
    :returns: list of video file paths
    :raises FileNotFoundError: if path does not exist or no videos found
    """
...
def process_one_video(
    ...
    with mp_pose.Pose(
        ...
        while True:
            ...
            kpts       = None
            phase_feat = np.zeros(10, dtype=np.float32)  # PHASE_FEATURE_DIM = 10

            if res.pose_landmarks:
                lm   = res.pose_landmarks.landmark
                kpts = landmarks_to_array(lm)
                phase_feat = extract_phase_features(lm)
    ...
    feature_seq = np.stack(all_phase_feats, axis=0)   # (T, 10)
    T = len(all_frames)
    ...
    if args.show:
        ...
        for t, frame in enumerate(all_frames):
            ...
            cv2.imshow("extract_bottom_phase", frame)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Extract squat bottom snippets using a trained PhaseTCN model."
    )
    ...
    ap.add_argument("--model",  required=True,
                    help="Path to trained phase model checkpoint (.pt).")
