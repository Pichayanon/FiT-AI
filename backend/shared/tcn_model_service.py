...
class PhaseAwareTCNModelService:
    def __init__(
        ...
        *,
        ...
        if phase_path and os.path.isfile(phase_path):
            self.phase_model, self.phase_window, self.phase_in_dim = load_phase_tcn(
                phase_path
            )
    ...
    def predict_bottom(
        self, window_features: np.ndarray
    ...
    def predict_stand(
        ...
        if (
            self.stand_model is None
            or self.stand_T is None
            or self.inv_labels_stand is None
        ):
            return "unknown", 0.0, np.array([])
        return predict_sequence_tcn(
            self.stand_model,
