...
Usage:
  # Folder-based split (recommended)
  python lunges/train_lunges_bottom.py \
    --data dataset/lunges/dataset_bottom \
    --out lunges/models/lunges_bottom_tcn.pt

  # (data folder should have train/ val/ test/ subfolders,
  #  or fall back to random split if not found)
"""
...
class LungeNPZDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        keypoints, _, label_name = load_npz(sample.path)
...
def train(args: argparse.Namespace) -> None:
    """Train a TCN for lunge bottom classification."""
    set_seed(args.seed)
    ...
    train_dataset = LungeNPZDataset(train_samples, label_map, T=args.T)
    val_dataset = LungeNPZDataset(val_samples, label_map, T=args.T)
    test_dataset = LungeNPZDataset(test_samples, label_map, T=args.T) if test_samples else None
    ...
    for epoch_index in range(1, int(args.epochs) + 1):
        model.train()
        loss_sum, correct_predictions, total = 0.0, 0, 0
        for feature_batch, label_batch in train_loader:
            ...
            batch_size = feature_batch.size(0)
            loss_sum += float(loss.item()) * batch_size
            correct_predictions += (logits.argmax(1) == label_batch).sum().item()
            total += batch_size

        tr_acc = correct_predictions / max(1, total)
    ...
    # Print final report for the best weights.
    ...
    checkpoint = {
        ...
        "meta": {
            "task": "lunges_bottom",
            "feature_set": "bottom_focused_42d",
            "feat_dim_detail": {
                "key_joint_xyz": 30, "scalars": 12, "total": BOTTOM_FEATURE_DIM
            },
...
def main() -> None:
    ...
    ap.add_argument("--data", required=True, help="folder with *.npz or train/val/test subfolders")
    ap.add_argument("--out", required=True, help="output .pt path e.g. lunges/models/lunges_bottom_tcn.pt")
